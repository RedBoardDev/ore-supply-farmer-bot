import type { LoggerPort } from '../infrastructure/logger.port';
...
export interface MiningCostStrategy {
  isEnabled(): boolean;
...
export class DefaultMiningCostStrategy implements MiningCostStrategy {
  ...
  constructor(
    private readonly config: MiningCostConfig,
  ...
  private async fetchEvPercent(): Promise<number | null> {
    try {
      ...
      if (!response.ok) {
        this.logger.warn(`Mining cost: API returned ${response.status}`);
        return null;
      }
      ...
      this.logger.warn(`Mining cost: API fetch failed (${(error as Error).message})`);
      return null;
    }
  }
}
