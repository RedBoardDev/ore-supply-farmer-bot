...
function computeExposureExistingSol(miner: Miner): number {
  return miner.deployed.reduce((acc, lamports) => acc + Number(lamports) / Number(LAMPORTS_PER_SOL), 0);
}

function computeNetOreValueSol(round: Round, netSolPerOre: number | null): number {
  if (!netSolPerOre || netSolPerOre <= 0) return 0;
  const motherlodeOre = Number(round.motherlode) / ORE_DECIMALS;
  return netSolPerOre * (1 + MOTHERLODE_TRIGGER_PROBABILITY * Math.max(motherlodeOre, 0));
}
...
function computeEvRatio(params: {
  ...
  const numerator =
    PROBABILITY_OF_WIN * (SOL_PAYOUT_FEE_FACTOR * (potFromOthersSol + exposureBeforeSol + stakeSol) + netOreValueSol);
...
export class EvStrategyService implements EvStrategyServicePort {
  private lastBestEvRatio: number | null = null;

  constructor(private readonly config: EvStrategyConfig) {}
  ...
  calculateDecisions(
    ...
    const netOreValueSol = this.config.includeOreInEv ? computeNetOreValueSol(round, netSolPerOre) : 0;
    ...
    for (const candidate of candidates) {
      ...
      if (evRatio <= minEvThreshold) {
        ...
        const adjustedStakeSol = Math.min(stakeLimit, Math.max(0, maxStakeSol));
  ...
  recalculateEv(params: {
    ...
    const netOreValueSol = this.config.includeOreInEv
      ? computeNetOreValueSol(round, orePrice?.netSolPerOre ?? null)
      : 0;
